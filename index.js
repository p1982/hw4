...
const bankAccount = {
    ...
    transfer(senderAccount, receiverAccount, amount) {
        if (
            typeof senderAccount !== 'object' || 
            typeof senderAccount.balance.value !== 'number' ||
            typeof receiverAccount !== 'object' || 
            typeof receiverAccount.balance.value !== 'number'
        ) {
        ...
        if (senderAccount.balance.value < amount) {
            throw new Error('Insufficient funds');
        }
        senderAccount.balance.value -= amount;
        receiverAccount.balance.value += amount;
    }
};

// Example usage:
const targetAccount = {
    balance: { value: 0 } // Initialize with any balance
};

const currentAccount = {
    balance: { value: 500 } // Initialize with any balance
};

try {
    bankAccount.transfer(currentAccount, targetAccount, 500);
    console.log("Transfer successful");
    console.log("Target account balance:", targetAccount.balance.value);
    console.log("Current account balance:", currentAccount.balance.value);
} catch (error) {
